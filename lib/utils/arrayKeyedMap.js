...
export function ArrayKeyedSet(initialEntries) {
  const rootMap = new Map();

  if (initialEntries) {
    for (const keyArray of initialEntries) {
      setValue(rootMap, keyArray, keyArray);
    }
  }
  ...
  const _this = {
    ...
    add: (keyArray) => { setValue(rootMap, keyArray, keyArray); return _this; },
    ...
    *entries() { yield* allKeys(rootMap) },
    *keys() { yield* allKeys(rootMap) },
    *values() { yield* allKeys(rootMap) },

    *[Symbol.iterator]() { yield* allKeys(rootMap) }
  };

  return _this;
}
ArrayKeyedSet[Symbol.species] = ArrayKeyedSet
